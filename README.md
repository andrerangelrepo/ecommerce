...
### Por que um `outputTemplate` customizado no sink de Console do Serilog?

Rodando a aplicação e observando o console (ver auditoria abaixo), percebi que o `TraceId` injetado pelo `ActivityEnricher` nunca aparecia no texto — o template padrão do Serilog só renderiza `Timestamp`/`Level`/`Message`/`Exception`, ignorando propriedades customizadas. A correlação existia nos dados mas era invisível pra quem lê o console. `appsettings.json` agora define `Serilog:WriteTo[0].Args.outputTemplate` incluindo `{TraceId}` no final de cada linha — sem precisar de formatter/pacote adicional, só configuração.

### Por que `MetricsConsoleExporterEnabled` é uma flag separada, desligada por padrão?

Rodei a aplicação parada por ~3 minutos, sem nenhuma requisição, e o console acumulou mais de 5000 linhas — quase tudo exportação periódica de métricas (Kestrel, memory pool, runtime), a cada 10s, mesmo sem tráfego algum. Diferente de um trace (que só existe quando uma requisição real acontece, então é naturalmente esparso e sempre relevante), métricas exportam num timer fixo independente de uso — um encaixe ruim pra um console de desenvolvimento, e às vezes intercalando no meio do log de uma requisição em andamento (uma exportação de métricas caiu literalmente entre `Handling GetOrdersQuery` e `Handled GetOrdersQuery`, um span de 300+ linhas totalmente desconexo do que se está tentando ler). `ConsoleExporterEnabled` agora controla só traces (mantém o nome/comportamento original); `MetricsConsoleExporterEnabled` (novo, `false` por padrão) controla métricas separadamente, e quando ligado, usa um intervalo de exportação de 30s em vez do padrão do SDK, para reduzir ainda mais o volume. Métricas continuam coletadas e disponíveis via OTLP a qualquer momento — só a exportação bruta pro console é que fica opt-in.

### Por que `ConsoleExporterEnabled` (traces) também virou `false` por padrão?

Numa auditoria de log posterior, rodando a aplicação e fazendo requisições reais, notei que o dump bruto de um span (`OpenTelemetry.Exporter.Console`) tem ~20 linhas, das quais só umas 6-8 carregam informação que varia por request (`TraceId`, `SpanId`, `ParentSpanId`, `DisplayName`, `Duration`, `Tags`) — o resto (`TraceFlags` sempre `Recorded`, o bloco `Resource associated with Activity` de 5 linhas, os campos `telemetry.sdk.*`) é boilerplate idêntico repetido em **todo** span da execução inteira. Diferente das métricas (removidas por rodar num timer, sem relação com uso), aqui o problema não é frequência — é densidade de informação: pra cada linha útil do Serilog (`Handling X`/`Handled X`/`HTTP responded`), o trace bruto adiciona várias linhas majoritariamente repetidas. Não há como o `OpenTelemetry.Exporter.Console` mostrar só os campos variáveis (é um exporter cru, sem modo compacto), e escrever um formatter customizado contrariaria a decisão de não criar infraestrutura de tracing própria. Consultado, o critério ficou: console limpo por padrão (só Serilog), com `ConsoleExporterEnabled: true` disponível a qualquer momento para inspecionar a árvore de spans quando for realmente necessário — o `TraceId` continua em toda linha do Serilog, então dá pra saber que o tracing existe e funciona sem precisar ver o dump completo o tempo todo.

### Por que o `outputTemplate` do Serilog agora mostra o offset UTC (`zzz`)?
